...
while True:
    ...
    if 680 + len(body_lines) * line_h + 150 <= H - 360 or body_size <= 34:
        break
    body_size -= 2
...
def render(f):
    ...
    for i in range(n_stars):
        star(d, MARGIN + 38 + i * 92, 400, 38, GOLD)
    ...
    if f >= STARS_END:
        ...
        shown = TITLE[:n]
        d.text((MARGIN, 492), shown, font=title_font, fill=INK)
        if f < BODY_START:
            cursor = (MARGIN + measurer.textlength(shown, font=title_font) + 10, 492 + 8, 72)
...
out = "feedback_friday_typing_vertical.mp4"
...
previews = {60: "prev_v_early.jpg", ATTR_START - 20: "prev_v_mid.jpg", ATTR_END + 20: "prev_v_end.jpg"}
